class Building():
	...
	def step(self, outside_temperature, heating_setpoint, cooling_setpoint):
		...
		def next_temperature(heating_cooling_power):
			self._determine_Demand(outside_temperature, heating_setpoint, cooling_setpoint)
	...
	def _determine_Demand(self, outside_temperature, heating_setpoint, cooling_setpoint):
		'''Checks whether there is a Demain for heading and cooling is within 1 degree
		
		Arguments:
			outside_temperature {float} -- The outside temperature in C
			heating_setpoint {float} -- The current heating point the furnace is set to
			cooling_setpoint {float} -- The current cooling point the A/C is set to
		'''
		# check whether the heater should be on
		self.heater_on = self.current_temperature < heating_setpoint 
		self.cooling_on = self.current_temperature > cooling_setpoint 
